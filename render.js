...
const preview = ImageGallery.childNodes[5]
...
options.forEach((option) => {
    option.addEventListener("click", () => {
        ...
        function filterAccordingToCatogery() {
            if (option.value == 'all') {
                displayProducts(prod);
                heading.innerText = "All Monuments"
            }
            if (option.value == 'east') {
                const filteredProd = prod.filter((data) => data.direction === 'East');
                displayProducts(filteredProd);
                heading.innerText = "East Sides"
            }
            if (option.value == 'west') {
                const filteredProd = prod.filter((data) => data.direction === 'West');
                displayProducts(filteredProd);
                heading.innerText = "West Sides"
            }
            if (option.value == 'north') {
                const filteredProd = prod.filter((data) => data.direction === 'North');
                displayProducts(filteredProd);
                heading.innerText = "North Sides"
            }
            if (option.value == 'south') {
                const filteredProd = prod.filter((data) => data.direction === 'South');
                displayProducts(filteredProd);
                heading.innerText = "South Sides"
            }
            if (option.value == 'wonders') {
                const filteredProd = prod.filter((data) => data.direction === 'Seven-Wonders');
                displayProducts(filteredProd);
                heading.innerText = "Seven wonders"
            }
        }
        filterAccordingToCatogery();
    });
});
...
inputs.forEach(select => {
    select.addEventListener('click', () => {
        function filterAccordingToCatogery() {
            ...
            if (select.value == 'west') {
                const filteredProd = prod.filter((data) => data.direction === 'West');
                displayProducts(filteredProd);
                heading.innerText = "West Sides"
            }
            if (select.value == 'north') {
                const filteredProd = prod.filter((data) => data.direction === 'North');
                displayProducts(filteredProd);
                heading.innerText = "North Sides"
            }
            if (select.value == 'south') {
                const filteredProd = prod.filter((data) => data.direction === 'South');
                displayProducts(filteredProd);
                heading.innerText = "South Sides"
            }
            if (select.value == 'wonders') {
                const filteredProd = prod.filter((data) => data.direction === 'Seven-Wonders');
                displayProducts(filteredProd);
                heading.innerText = "Seven wonders"
            }
